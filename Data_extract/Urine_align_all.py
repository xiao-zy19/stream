...
import argparse
import time
from utils import patient_id_age, align_data, urine
...
folder_path = '/home/en580-zxia028/output/urine'
...
if batch_size.lower() == 'all':
    ...
    file_name = 'urine_all.csv'
    print('batch size: ', batch_size)
    print('now processing all data')
else:
    batch_size = int(batch_size)  # turn batch_size into integer
    patient_batch = [patient_id[i:i + batch_size] for i in range(0, len(patient_id), batch_size)]
    file_name = 'urine_full_' + str(batch) + '.csv'
    print('batch size: ', batch_size)
    print('now processing batch: ', batch, '/', len(patient_batch)-1)

print(patient_batch[batch].shape)
# extract heart rate data
Urine, _ = urine(patient_batch[batch])
data_full, data_full_index = align_data(patient_batch[batch], patient_offset, Urine, graph=False)
